...
class TDMPC():
	...
	def update(self, replay_buffer, step):
		...
		z_aug = self.model.h(self.aug(obs))
		zs = [z.detach()]
		...
		for t in range(self.cfg.horizon):
			...
			Q1_aug, Q2_aug = self.model.Q(z_aug, action[t])

			z, reward_pred = self.model.next(z, action[t])
			z_aug, reward_pred_aug = self.model.next(z_aug, action[t])


			with torch.no_grad():
				next_obs = self.aug(next_obses[t])
				next_obs_aug = self.aug(next_obses[t])
				# regularized z
				next_z = self.model_target.h(next_obs)
				next_z_aug = self.model_target.h(next_obs_aug)

				td_target = self._td_target(next_obs, reward[t])
				td_target_aug = self._td_target(next_obs_aug, reward[t])

				td_target = (td_target + td_target_aug) / 2
				next_z = (next_z + next_z_aug) / 2

			zs.append(z.detach())

			# Losses
			rho = (self.cfg.rho ** t)
			consistency_loss += rho * torch.mean((h.mse(z, next_z) + h.mse(z_aug, next_z)), dim=1, keepdim=True)
			# consistency_loss += rho * torch.mean(h.mse(z, next_z), dim=1, keepdim=True)
			reward_loss += rho * (h.mse(reward_pred, reward[t]) + h.mse(reward_pred_aug, reward[t]))
			# reward_loss += rho * h.mse(reward_pred, reward[t])
			value_loss += rho * (h.mse(Q1, td_target) + h.mse(Q2, td_target) + h.mse(Q1_aug, td_target) + h.mse(Q2_aug, td_target))
			priority_loss += rho * (h.l1(Q1, td_target) + h.l1(Q2, td_target) + h.l1(Q1_aug, td_target) + h.l1(Q2_aug, td_target))
